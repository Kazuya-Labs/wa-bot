...
async function GroupUpdate(xzuya, update, store) {
	try {
		for (let n of update) {
	...
	try {
		if (
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await xzuya.profilePictureUrl(n, "image");
				} catch {
					profile =
...
│ • *A s k o t    :* 
└───────────────⬣
*JANGAN LUPA BACA DEKRIPSI*
				`;
					await xzuya.sendMessage(id, {
						text: `Welcome @${
							n.split("@")[0]
						}\n _Jangan lupa introo_\n ${intro}`,
						contextInfo: {
							mentionedJid: [n],
							forwardingScore: 10,
							isForwarded: true,
...
async function LoadDataBase(xzuya, m) {
	try {
...
async function MessagesUpsert(xzuya, message, store) {
	try {
		let botNumber = await xzuya.decodeJid(xzuya.user.id);
		const msg = message.messages[0];
		if (
			store.groupMetadata &&
			Object.keys(store.groupMetadata).length === 0
		)
			store.groupMetadata = await xzuya.groupFetchAllParticipating();
		const type = msg.message
			? getContentType(msg.message) || Object.keys(msg.message)[0]
			: "";
		if (!msg.key.fromMe && !msg.message && message.type === "notify")
			return;
		const m = await Serialize(xzuya, msg, store);
		require("../xzuya")(xzuya, m, message, store);
		if (
			...
			let apb = await generateWAMessage(
				m.chat,
				{
					text: JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson)
						.id,
					mentions: m.mentionedJid
				},
				{
					userJid: xzuya.user.id,
					quoted: m.quoted
				}
			);
			apb.key = msg.key;
			apb.key.fromMe = areJidsSameUser(m.sender, xzuya.user.id);
			...
			xzuya.ev.emit("messages.upsert", pbr);
		}
		if (
			global.db.set &&
			global.db.set[botNumber] &&
			...
			if (msg.key.remoteJid === "status@broadcast") {
				await xzuya.readMessages([msg.key]);
...
async function Solving(xzuya, store) {
	xzuya.public = true;

	xzuya.serializeM = (m) => MessagesUpsert(xzuya, m, store);
	...
	xzuya.getName = (jid, withoutContact = false) => {
		const id = xzuya.decodeJid(jid);
		if (id.endsWith("@g.us")) {
			const groupInfo =
				store.contacts[id] || xzuya.groupMetadata(id) || {};
	...
	xzuya.sendContact = async (jid, kon, quoted = "", opts = {}) => {
		let list = [];
		for (let i of kon) {
			list.push({
				displayName: await xzuya.getName(i + "@s.whatsapp.net"),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await xzuya.getName(
					i + "@s.whatsapp.net"
				)}\nFN:${await xzuya.getName(
					i + "@s.whatsapp.net"
				)}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await xzuya.getName(i + '@s.whatsapp.net')}\nFN:${await xzuya.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/xzuya_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			});
		}
		xzuya.sendMessage(
			jid,
	...
	xzuya.profilePictureUrl = async (jid, type = "image", timeoutMs) => {
		const result = await xzuya.query(
			{
				tag: "iq",
				attrs: {
	...
	xzuya.sendPoll = (jid, name = "", values = [], selectableCount = 1) => {
		return xzuya.sendMessage(jid, {
			poll: { name, values, selectableCount }
		});
	};

	xzuya.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes("gif")) {
				return xzuya.sendMessage(
					jid,
					{
						video: res.data,
						caption: caption,
	...
	xzuya.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await xzuya.sendMessage(
	...
	xzuya.sendTextMentions = async (jid, text, quoted, options = {}) =>
		xzuya.sendMessage(
			jid,
			{
				text: text,
	...
	xzuya.sendAsSticker = async (jid, path, quoted, options = {}) => {
	...
	xzuya.downloadAndSaveMediaMessage = async (
		message,
		filename,
		...
		const trueFileName = attachExtension
			? `./database/sampah/${filename ? filename : Date.now()}.${
					type.ext
			  }`
			: filename;
	...
	xzuya.sendMedia = async (
		...
		const { mime, data, filename } = await xzuya.getFile(path, true);
	...
	xzuya.sendButtonMsg = async (
		...
		const hasil = await xzuya.relayMessage(msg.key.remoteJid, msg.message, {
			messageId: msg.key.id
		});
		return hasil;
	};

	xzuya.sendCarouselMsg = async (
		jid,
		body = "",
		footer = "",
		cards = [],
...
async function Serialize(xzuya, m, store) {
	const botNumber = xzuya.decodeJid(xzuya.user.id);
	if (!m) return m;
	if (m.key) {
		m.id = m.key.id;
		...
		m.isBot =
			["HSK", "BAE", "B1E", "3EB0", "WA"].some(
				(a) =>
					m.id.startsWith(a) &&
					[12, 16, 20, 22, 40].includes(m.id.length)
			) || false;
		m.isGroup = m.chat.endsWith("@g.us");
		m.sender = xzuya.decodeJid(
			(m.fromMe && xzuya.user.id) ||
				m.participant ||
				m.key.participant ||
				m.chat ||
				""
		...
		if (m.isGroup) {
			m.metadata =
				...
				(await xzuya.groupMetadata(m.chat));
			m.admins = m.metadata.participants.reduce(
				(a, b) =>
					(b.admin ? a.push({ id: b.id, admin: b.admin }) : [...a]) &&
					a,
				[]
			);
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.isBot = m.quoted.id
				...
				: false;
			m.quoted.sender = xzuya.decodeJid(m.msg.contextInfo.participant);
			m.quoted.fromMe =
				m.quoted.sender === xzuya.decodeJid(xzuya.user.id);
			m.quoted.text =
				m.quoted.caption ||
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, xzuya);
				return await Serialize(xzuya, q, store);
			};
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(
					xzuya.decodeJid(m.msg?.contextInfo?.participant),
					xzuya.decodeJid(xzuya?.user?.id)
				),
			...
			m.quoted.delete = () => {
				xzuya.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
